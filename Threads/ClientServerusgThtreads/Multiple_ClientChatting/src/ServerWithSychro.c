...
//	printf("sock id :%d\n",new_socket[0]);	
	...
	if(new_socket[1] < 0) { 
		...
		printf("Connection is Esatablished\n");
	new_socket[2] = accept(server_fd, (struct sockaddr *)&address,(socklen_t*)&addrlen); 
	if(new_socket[2] < 0) { 
		perror("accept"); 
		exit(EXIT_FAILURE); 
	}
		...
		printf("Connection is Esatablished\n");
	new_socket[3] = accept(server_fd, (struct sockaddr *)&address,(socklen_t*)&addrlen); 
	if(new_socket[3] < 0) { 
		perror("accept"); 
		exit(EXIT_FAILURE); 
	}
...
//	printf("Base address of socket : %d\n",new_socket);
